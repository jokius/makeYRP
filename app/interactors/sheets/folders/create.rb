...
class Sheets::Folders::Create
  ...
  def fetch_parent(input)
    folder = Games::SheetFolder.find(input[:parent_id])
    if folder
      Success(folder)
    else
  ...
  def create(parent:, params:)
    folder = parent.children.build(params.merge(game_id: parent.game_id))

    if folder.save
      Success(folder.folder_struct(sheets: [], children: [], path_loaded: true))
    else
