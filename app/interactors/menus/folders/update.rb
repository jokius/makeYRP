...
class Menus::Folders::Update
  ...
  def fetch_folder(input)
    folder = ::Menus::ItemFolder.find_by(id: input.delete(:id))
    if folder
      Success(folder)
    else
      Failure(message: 'menu folder not found')
    end
  end

  def update(folder:, input:)
    if folder.update(input)
      Success(folder.folder_struct(items: folder.items, children: folder.children, path_loaded: true))
    else
